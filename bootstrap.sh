...
if [ -n "$GOOGLE_OAUTH_CLIENT_ID" ] && [ -n "$GOOGLE_OAUTH_CLIENT_SECRET" ] && [ -n "$GHCR_PAT" ]; then
    ...
    run_quiet sh -c "kubectl get secret traefik-me-tls -n traefik -o jsonpath='{.data.ca\.crt}' | base64 -d > /tmp/traefik-me-ca.crt" || die "Failed to extract traefik-me CA cert"
    run_quiet sh -c "kubectl create configmap traefik-me-ca --from-file=ca.crt=/tmp/traefik-me-ca.crt -n toolhive-system --dry-run=client -o yaml | kubectl apply -f -" || die "Failed to create traefik-me-ca ConfigMap"
