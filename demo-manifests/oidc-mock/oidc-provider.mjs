/**
 * Dev-only OIDC provider for local testing.
 *
 * IMPORTANT: This is NOT for production use. It:
 * - Auto logs-in a test user and auto-consents
 * - Issues refresh tokens unconditionally (issueRefreshToken = true)
 * - Uses 1-hour AccessToken TTL for demo convenience
 * - Uses in-memory adapter and generated signing keys
 */
import { config } from 'dotenv';
import Provider from 'oidc-provider';

config();
config({ path: '.env.local' });

const ISSUER = process.env.OIDC_ISSUER_URL || 'http://localhost:4000';
// Allow explicit PORT override for cases where the advertised issuer URL differs from the listen port
// (e.g., Kubernetes where external URL is different from internal container port)
const PORT = process.env.PORT
  ? parseInt(process.env.PORT, 10)
  : new URL(ISSUER).port || 4000;
const CLIENT_ID = process.env.OIDC_CLIENT_ID || 'better-auth-dev';
const CLIENT_SECRET =
  process.env.OIDC_CLIENT_SECRET || 'dev-secret-change-in-production';
const UI_HOSTNAME = process.env.UI_HOSTNAME || 'localhost:3000';
const DEBUG = process.env.DEBUG === 'true' || false;

// Simple in-memory account storage
const accounts = {
  'test-user': {
    accountId: 'test-user',
    email: 'test@example.com',
    email_verified: true,
    name: 'Test User',
  },
};

// Configuration
const configuration = {
  // Trust proxy headers (X-Forwarded-Proto, etc.)
  proxy: true,
  clients: [
    {
      client_id: CLIENT_ID,
      client_secret: CLIENT_SECRET,
      redirect_uris: [
        // Better Auth genericOAuth uses /oauth2/callback/:providerId
        'http://localhost:3000/api/auth/oauth2/callback/oidc',
        'http://localhost:3001/api/auth/oauth2/callback/oidc',
        'http://localhost:3002/api/auth/oauth2/callback/oidc',
        'http://localhost:3003/api/auth/oauth2/callback/oidc',
        'http://localhost:3000/api/auth/oauth2/callback/okta',
        `https://${UI_HOSTNAME}/api/auth/oauth2/callback/okta`,
        `https://${UI_HOSTNAME}/api/auth/oauth2/callback/oidc`,
      ],
      // Post-logout redirect URIs for RP-Initiated Logout
      post_logout_redirect_uris: [
        'http://localhost:3000/signin',
        'http://localhost:3001/signin',
        'http://localhost:3002/signin',
        'http://localhost:3003/signin',
        `https://${UI_HOSTNAME}/signin`,
      ],
      response_types: ['code'],
      grant_types: ['authorization_code', 'refresh_token'],
      token_endpoint_auth_method: 'client_secret_post',
    },
  ],
  cookies: {
    keys: ['some-secret-key-for-dev'],
  },
  findAccount: async (_ctx, id) => {
    const account = accounts[id];
    if (!account) return undefined;

    return {
      accountId: id,
      async claims() {
        return {
          sub: id,
          email: account.email,
          email_verified: account.email_verified,
          name: account.name,
        };
      },
    };
  },
  // Simple interaction - auto-login for dev
  interactions: {
    url(_ctx, interaction) {
      return `/interaction/${interaction.uid}`;
    },
  },
  features: {
    // Disable built-in dev interactions in favor of our custom auto-login
    // and auto-consent middleware used for local testing.
    devInteractions: { enabled: false },
    // Enable RP-Initiated Logout for sign-out flow
    rpInitiatedLogout: {
      enabled: true,
      // Auto-confirm logout for dev (skip confirmation page)
      logoutSource: async (ctx, form) => {
        ctx.body = `<!DOCTYPE html>
          <html><head><title>Logging out...</title></head>
          <body>
            ${form}
            <script>document.forms[0].submit();</script>
          </body></html>`;
      },
    },
  },
  // Explicitly declare supported scopes, including offline_access for refresh tokens
  // Scopes supported by the dev provider (app requests offline_access in dev and prod)
  scopes: ['openid', 'email', 'profile', 'offline_access'],
  claims: {
    email: ['email', 'email_verified'],
    profile: ['name'],
  },
  // Include claims in ID token, not just in UserInfo endpoint
  // Some clients (like Better Auth) may expect claims in the ID token
  conformIdTokenClaims: false,
  ttl: {
    // Access tokens valid for 1 hour
    AccessToken: 3600, // seconds
    RefreshToken: 86400 * 30, // 30 days
  },
  // Dev-only: always issue refresh tokens to make the flow reliable locally
  issueRefreshToken: async () => true,
};

const oidc = new Provider(ISSUER, configuration);

// CRITICAL: Tell Koa to trust proxy headers (X-Forwarded-Proto, etc.)
// The Provider instance IS the Koa app
oidc.proxy = true;

// Log incoming requests
oidc.use(async (ctx, next) => {
  const start = Date.now();

  if (DEBUG) {
    const headers = {
      'user-agent': ctx.get('user-agent'),
      'x-forwarded-for': ctx.get('x-forwarded-for'),
      'x-forwarded-proto': ctx.get('x-forwarded-proto'),
      'x-forwarded-host': ctx.get('x-forwarded-host'),
      'x-forwarded-port': ctx.get('x-forwarded-port'),
      referer: ctx.get('referer'),
    };
    console.log(
      `[${new Date().toISOString()}] --> ${ctx.method} ${ctx.path}`,
      JSON.stringify(headers)
    );

    // Log what Koa is detecting for .well-known endpoint
    if (ctx.path === '/.well-known/openid-configuration') {
      console.log('[DEBUG] Koa ctx.protocol:', ctx.protocol);
      console.log('[DEBUG] Koa ctx.host:', ctx.host);
      console.log('[DEBUG] Koa ctx.origin:', ctx.origin);
      console.log('[DEBUG] Koa ctx.href:', ctx.href);
    }
  } else {
    console.log(`[${new Date().toISOString()}] --> ${ctx.method} ${ctx.path}`);
  }

  await next();
  const duration = Date.now() - start;
  console.log(
    `[${new Date().toISOString()}] <-- ${ctx.method} ${ctx.path} ${
      ctx.status
    } (${duration}ms)`
  );

  if (DEBUG) {
    // Log token responses
    if (ctx.path === '/token' && ctx.status === 200 && ctx.body) {
      console.log(
        '[Token Response]',
        JSON.stringify(
          {
            access_token: ctx.body.access_token ? 'present' : 'missing',
            refresh_token: ctx.body.refresh_token ? 'present' : 'missing',
            id_token: ctx.body.id_token ? 'present' : 'missing',
            token_type: ctx.body.token_type,
            expires_in: ctx.body.expires_in,
            scope: ctx.body.scope,
          },
          null,
          2
        )
      );

      // Decode and log ID token claims if present
      if (ctx.body.id_token) {
        try {
          const payload = ctx.body.id_token.split('.')[1];
          const decoded = JSON.parse(Buffer.from(payload, 'base64').toString());
          console.log('[ID Token Claims]', JSON.stringify(decoded, null, 2));
        } catch (err) {
          console.error('[ID Token] Failed to decode:', err.message);
        }
      }
    }

    // Log JWKS requests
    if (ctx.path === '/jwks') {
      console.log('[JWKS] Keys requested by client');
    }

    // Log UserInfo responses
    if (ctx.path === '/me' && ctx.status === 200 && ctx.body) {
      console.log('[UserInfo Response]', JSON.stringify(ctx.body, null, 2));
    }
  }
});

// Simple interaction endpoint for dev - auto-login as test-user
oidc.use(async (ctx, next) => {
  if (ctx.path.startsWith('/interaction/')) {
    const _uid = ctx.path.split('/')[2];
    const interaction = await oidc.interactionDetails(ctx.req, ctx.res);

    if (interaction.prompt.name === 'login') {
      // Auto-login as test-user for dev
      await oidc.interactionFinished(
        ctx.req,
        ctx.res,
        {
          login: {
            accountId: 'test-user',
          },
        },
        { mergeWithLastSubmission: false }
      );
      return;
    }

    if (interaction.prompt.name === 'consent') {
      // Auto-consent for dev
      const grant = new oidc.Grant({
        accountId: interaction.session.accountId,
        clientId: interaction.params.client_id,
      });

      // Grant requested scopes intersected with allowed ones
      const requestedScopes = interaction.params.scope
        ?.split(' ')
        .filter(Boolean) || ['openid', 'email', 'profile'];
      const allowedScopes = ['openid', 'email', 'profile', 'offline_access'];
      const grantedScopes = requestedScopes.filter((s) =>
        allowedScopes.includes(s)
      );
      // Dev-only provider already issues refresh tokens unconditionally via
      // `issueRefreshToken`, so we don't need to force-add offline_access here.
      grant.addOIDCScope(grantedScopes.join(' '));

      await grant.save();

      await oidc.interactionFinished(
        ctx.req,
        ctx.res,
        {
          consent: {
            grantId: grant.jti,
          },
        },
        { mergeWithLastSubmission: true }
      );
      return;
    }
  }
  await next();
});

oidc.listen(PORT, () => {
  console.log(`🔐 OIDC Provider running at ${ISSUER}`);
  console.log(`📝 Client ID: ${CLIENT_ID}`);
  console.log(`🔑 Client Secret: ${CLIENT_SECRET}`);
  console.log(`👤 Test user: test@example.com`);
  console.log(
    `\n⚙️  Update your .env.local with:\nOIDC_CLIENT_ID=${CLIENT_ID}\nOIDC_CLIENT_SECRET=${CLIENT_SECRET}\nOIDC_ISSUER_URL=${ISSUER}`
  );
});
